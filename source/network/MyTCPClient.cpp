...
MyTCPClient::MyTCPClient(int fd, SSL* sl, std::string addr) : MyClientSocket()
{
	socket_fd = fd;
	Address = addr;
	ssl = sl;
...
StackErrorCode MyTCPClient::Connect(std::string addr, int port)
{
	...
	try	//TODO : certificate, key를 파일이 아닌 직접 생성하는 방법도 찾아보자. 파일을 사용하면 Connector, Connectee가 같은 인증서/키를 사용하게됨.
	{
		...
		if(!ctx)
			throw StackErrorCode(GetSSLError(), __STACKINFO__);
	...
	SSL_CTX_free(ctx);
	if(!ssl)
		return StackErrorCode(GetSSLError(), __STACKINFO__);
...
ErrorCode MyTCPClient::GetSSLErrorFromRet(int ret)
{
