...
MyTCPServer::MyTCPServer(int p) : MyServerSocket(p)
{
	...
	if(server_fd < 0)
		throw StackTraceExcept("MyTCPServer::Open()::socket() Failed (" + std::to_string(errno) + ")", __STACKINFO__);
	if(setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, (const int*)&opt_reuseaddr, sizeof(opt_reuseaddr)) < 0)
		throw StackTraceExcept("MyTCPServer::Open()::setsockopt() Failed (" + std::to_string(errno) + ")", __STACKINFO__);
	...
	if(bind(server_fd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0)
		throw StackTraceExcept("MyTCPServer::Open()::bind() Failed (" + std::to_string(errno) + ")", __STACKINFO__);
	if(listen(server_fd, LISTEN_SIZE) < 0)
		throw StackTraceExcept("MyTCPServer::Open()::listen() Failed (" + std::to_string(errno) + ")", __STACKINFO__);
}
...
Expected<std::shared_ptr<MyClientSocket>, int> MyTCPServer::Accept()
{
